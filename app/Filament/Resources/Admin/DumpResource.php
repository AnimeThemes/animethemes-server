...
class DumpResource extends BaseResource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                TextInput::make(Dump::ATTRIBUTE_PATH)
    ...
    public static function table(Table $table): Table
    {
        return parent::table($table)
            ->recordUrl('')
            ->columns([
                TextColumn::make(Dump::ATTRIBUTE_ID)
                    ->label(__('filament.fields.base.id')),

                TextColumn::make(Dump::ATTRIBUTE_PATH)
                    ->label(__('filament.fields.dump.path'))
                    ->searchable()
                    ->copyableWithMessage(),
        ...
        return $schema
            ->components([
                Section::make(static::getRecordTitle($schema->getRecord()))
                    ->schema([
                        TextEntry::make(Dump::ATTRIBUTE_ID)
                            ->label(__('filament.fields.base.id')),
