...
class ThemeResource extends BaseResource
{
     ...
     */
    protected static ?string $model = AnimeTheme::class;
                ...
                Tabs::make('Tabs')
                    ->tabs([
                        Tab::make('theme')
                            ...
                            ->schema([
                                BelongsTo::make(AnimeTheme::ATTRIBUTE_ANIME)
                                ...
                                BelongsTo::make(AnimeTheme::ATTRIBUTE_GROUP)
                        ...
                        Tab::make('song')
                            ...
                            ->schema([
                                BelongsTo::make(AnimeTheme::ATTRIBUTE_SONG)
    ...
    public static function table(Table $table): Table
    {
        return parent::table($table)
            ->columns([
                BelongsToColumn::make(AnimeTheme::RELATION_ANIME, AnimeResource::class)
                    ->hiddenOn(ThemeAnimeRelationManager::class),

                TextColumn::make(AnimeTheme::ATTRIBUTE_ID)
                    ->label(__('filament.fields.base.id')),

                TextColumn::make(AnimeTheme::ATTRIBUTE_TYPE)
                    ->label(__('filament.fields.anime_theme.type.name'))
                    ->formatStateUsing(fn (ThemeType $state): ?string => $state->localize()),

                TextColumn::make(AnimeTheme::ATTRIBUTE_SEQUENCE)
                    ->label(__('filament.fields.anime_theme.sequence.name')),

                TextColumn::make(AnimeTheme::ATTRIBUTE_SLUG)
                    ->label(__('filament.fields.anime_theme.slug.name')),

                BelongsToColumn::make(AnimeTheme::RELATION_GROUP, GroupResource::class),

                BelongsToColumn::make(AnimeTheme::RELATION_SONG, SongResource::class)
                    ...
                    ->searchable(true, function (Builder $query, string $search): void {
                        ...
                        $query->whereHas(AnimeTheme::RELATION_SONG, function (Builder $query) use ($songs): void {
        ...
        return $schema
            ->components([
                Section::make(static::getRecordTitle($schema->getRecord()))
                    ->schema([
                        TextEntry::make(AnimeTheme::ATTRIBUTE_ID)
                            ->label(__('filament.fields.base.id')),

                        BelongsToEntry::make(AnimeTheme::RELATION_ANIME, AnimeResource::class),

                        TextEntry::make(AnimeTheme::ATTRIBUTE_TYPE)
                            ->label(__('filament.fields.anime_theme.type.name'))
                            ->formatStateUsing(fn (ThemeType $state): ?string => $state->localize()),

                        TextEntry::make(AnimeTheme::ATTRIBUTE_SEQUENCE)
                            ->label(__('filament.fields.anime_theme.sequence.name')),

                        TextEntry::make(AnimeTheme::ATTRIBUTE_SLUG)
                            ->label(__('filament.fields.anime_theme.slug.name')),

                        BelongsToEntry::make(AnimeTheme::RELATION_GROUP, GroupResource::class)
                            ->label(__('filament.resources.singularLabel.group')),

                        BelongsToEntry::make(AnimeTheme::RELATION_SONG, SongResource::class)
                            ->label(__('filament.resources.singularLabel.song')),
                    ])
                    ->columns(3),
                ...
                Section::make(__('filament.resources.singularLabel.song'))
                    ->schema([
                        RepeatableEntry::make(AnimeTheme::RELATION_PERFORMANCES)
                            ->label('')
    ...
    public static function getFilters(): array
    {
        return [
            QueryBuilder::make()
                ->constraints([
                    SelectConstraint::make(AnimeTheme::ATTRIBUTE_TYPE)
                        ->label(__('filament.fields.anime_theme.type.name'))
                        ->options(ThemeType::class)
                        ->multiple(),

                    NumberConstraint::make(AnimeTheme::ATTRIBUTE_SEQUENCE)
                        ->label(__('filament.fields.anime_theme.sequence.name')),
            ...
            Filter::make(ThemeType::IN->localize())
                ...
                ->query(fn (Builder $query) => $query->whereNot(AnimeTheme::ATTRIBUTE_TYPE, ThemeType::IN->value))
                ->default(true),
