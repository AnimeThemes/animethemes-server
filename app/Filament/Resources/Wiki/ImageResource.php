...
class ImageResource extends BaseResource
{
    ...
    public static function table(Table $table): Table
    {
        return parent::table($table)
            ->columns([
                Stack::make([
                    TextColumn::make(Image::ATTRIBUTE_ID)
                        ->label(__('filament.fields.base.id')),

                    TextColumn::make(Image::ATTRIBUTE_FACET)
                        ->label(__('filament.fields.image.facet.name'))
        ...
        return $schema
            ->components([
                Section::make(static::getRecordTitle($schema->getRecord()))
                    ->schema([
                        TextEntry::make(Image::ATTRIBUTE_ID)
                            ->label(__('filament.fields.base.id')),

                        TextEntry::make(Image::ATTRIBUTE_FACET)
                            ->label(__('filament.fields.image.facet.name'))
                            ->formatStateUsing(fn (ImageFacet $state): ?string => $state->localize()),
    ...
    public static function getFilters(): array
    {
        return [
            QueryBuilder::make()
                ->constraints([
                    SelectConstraint::make(Image::ATTRIBUTE_FACET)
                        ->label(__('filament.fields.image.facet.name'))
                        ->options(ImageFacet::class)
                        ->multiple(),
