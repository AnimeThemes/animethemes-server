...
class PermissionResource extends BaseResource
{
    ...
    {
        return Permission::ATTRIBUTE_NAME;
    }
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                TextInput::make(Permission::ATTRIBUTE_NAME)
    ...
    public static function table(Table $table): Table
    {
        return parent::table($table)
            ->recordUrl(fn (Permission $record): string => static::getUrl('view', ['record' => $record]))
            ->columns([
                TextColumn::make(Permission::ATTRIBUTE_ID)
                    ->label(__('filament.fields.base.id')),

                TextColumn::make(Permission::ATTRIBUTE_NAME)
                    ->label(__('filament.fields.permission.name'))
                    ->searchable()
                    ->copyableWithMessage(),
            ]);
        ...
        return $schema
            ->components([
                Section::make(static::getRecordTitle($schema->getRecord()))
                    ->schema([
                        TextEntry::make(Permission::ATTRIBUTE_ID)
                            ->label(__('filament.fields.base.id')),

                        TextEntry::make(Permission::ATTRIBUTE_NAME)
                            ->label(__('filament.fields.permission.name'))
                            ->copyableWithMessage(),
                    ]),
