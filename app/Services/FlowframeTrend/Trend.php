...
use App\Services\FlowframeTrend\Adapters\MySqlAdapter;
use App\Services\FlowframeTrend\Adapters\PgsqlAdapter;
use App\Services\FlowframeTrend\Adapters\SqliteAdapter;
...
class Trend
{
    ...
    {
        $values = $values->map(fn ($value): TrendValue => new TrendValue(
            date: $value->{$this->dateAlias},
            aggregate: $value->aggregate,
        ));

        $placeholders = $this->getDatePeriod()->map(
            fn (CarbonInterface $date): TrendValue => new TrendValue(
